...
class DimonaServiceTest(TestCase):
    ...
    @patch('apps.legal.services.dimona_service.Dimona.objects.filter')
    @patch('apps.legal.services.dimona_service.DimonaService._make_post')
    def test_create_dimona(self, mock_make_post, mock_dimona_filter, mock_get_type_for_user):
        ...
        with patch('apps.legal.services.dimona_service.FormattingUtil.to_user_timezone', return_value=datetime.datetime.now()):
