...
class ContractUtil:
    ...
    def get_context(application):
        ...
        if start.hour < 12:
            start_time_morning = FormattingUtil.to_readable_time(
                application.job.start_time
            )
            if end.hour < 12:
                end_time_morning = FormattingUtil.to_readable_time(
                    application.job.end_time
                )
            else:
                end_time_morning = "12:00"
        else:
            start_time_afternoon = FormattingUtil.to_readable_time(
                application.job.start_time
            )
            end_time_afternoon = FormattingUtil.to_readable_time(
                application.job.end_time
            )
        ...
        duration = FormattingUtil.to_readable_duration(
            application.job.end_time - application.job.start_time,
        )
        ...
        if application.worker.worker_profile.date_of_birth is not None:
            birth_date = FormattingUtil.to_full_date(
                application.worker.worker_profile.date_of_birth
            )

        signature_path = os.path.join(
            settings.BASE_DIR, "templates", "contracts", "signature.png"
        )
    ...
    def generate_contract(application: JobApplication):
        ...
        template_name = template_mapping.get(
            (
                application.job.customer.customer_profile.special_committee,
                application.worker.worker_profile.worker_type,
            )
        )
        ...
        with open(contract_path, "rb") as result_file:
            ...
            application.contract.save(
                f"{application.id}_contract.pdf", django_file, save=True
            )
