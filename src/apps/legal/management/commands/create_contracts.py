...
from apps.jobs.models import (
    JobApplication,
    JobApplicationState,
)  # Adjust the import path to your actual JobApplication model
from apps.legal.utils import (
    ContractUtil,
)  # Adjust the import path to your actual ContractUtil
    ...
    def handle(self, *args, **kwargs):
        ...
        for application in job_applications_without_contracts:
            try:
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"Contract created for application ID: {application.id}"
                    )
                )
                ...
                self.stdout.write(
                    self.style.ERROR(
                        f"Failed to create contract for application ID: {application.id}, Error: {str(e)}"
                    )
                )
