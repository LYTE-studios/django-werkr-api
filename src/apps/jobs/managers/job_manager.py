...
from apps.notifications.managers.notification_manager import (
    NotificationManager,
...
class JobManager(models.Manager):
    ...
    def deny_application(application: JobApplication):
        ...
        send_new_push = (
            application.job.max_workers - application.job.selected_workers == 0
        )
        ...
        if (job.max_workers - selected_workers) > 0 and send_new_push:
            JobManager._send_job_notification(
                job=job,
    ...
    def _notify_approved_worker(application: JobApplication):
        ...
        # Customer email
        SelectedWorkerTemplate().send(
            recipients=[{"Email": application.job.customer.email}],
            data={
                "title": job.title,
                "weekday": FormattingUtil.to_day_of_the_week(start),
                "date": FormattingUtil.to_date(start),
    ...
    def remove_unselected_workers(job: Job):
        applications = JobApplication.objects.filter(
            job_id=job.id, application_state=JobApplicationState.pending
        )
    ...
    def calculate_selected_workers(application: JobApplication):
        ...
        count = JobApplication.objects.filter(
            job_id=application.job.id, application_state=JobApplicationState.approved
        ).count()
    ...
    def _send_job_notification(
        ...
        asyncio.create_task(
            create_global_notification(
                title, description, image_url=None, send_push=True
            )
        )

    @staticmethod
    def get_overlap_applications(
        application: JobApplication,
        state: JobApplicationState = JobApplicationState.pending,
    ):
        date_range = [
            application.job.start_time - datetime.timedelta(hours=3),
            application.job.end_time + datetime.timedelta(hours=3),
        ]

        overlap_applications = JobApplication.objects.filter(
    ...
    @staticmethod
    def get_end_overlap_applications(
        application: JobApplication,
        state: JobApplicationState = JobApplicationState.pending,
    ):
        date_range = [
