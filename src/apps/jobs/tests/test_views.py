...
class AllUpcomingJobsViewTest(TestCase):
    ...
    def test_get_all_upcoming_jobs(self, mock_get_upcoming_jobs):
        ...
        response = self.client.get(
            self.url, {"start": "2023-01-01", "end": "2023-12-31"}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data, {k_jobs: self.jobs})
...
class SignTimeRegistrationViewTest(TestCase):
    ...
    def test_post_sign_time_registration_bad_request(self, mock_sign_time_registration):
        mock_sign_time_registration.side_effect = DeserializationException(
            "Invalid data"
        )
        response = self.client.post(self.url, {}, format="json")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    @patch.object(JobService, "sign_time_registration")
    def test_post_sign_time_registration_internal_server_error(
        self, mock_sign_time_registration
    ):
...
class DirectionsViewTest(APITestCase):
    ...
    def test_get_directions_success(self, mock_fetch_directions):
        mock_response = {
            "routes": [{"distanceMeters": 1000, "polyline": "encoded_polyline"}]
        }
